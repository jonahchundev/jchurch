...
import type { GroupImportResult } from "../api/types";
import { splitGroupImportRow } from "../csvSchema";
import { Button, Heading, Label, Notice, Row, Sheet, styles } from "../ui";

type Busy = "" | "export" | "template" | "import";

export default function GroupsImportExport({
  churchId,
  onClose,
  onImported,
}: {
  ...
  async function importText(text: string) {
    ...
    try {
      ...
      const rows = parsed.data.map(splitGroupImportRow);
      const outcome = await api.importGroups(churchId, rows);
      setResult(outcome);
  ...
  return (
    <Sheet title="Import / export groups" onClose={onClose} busy={busy !== ""}>
      <View style={styles.stack}>
        ...
        <Button
          ...
          busy={busy === "export"}
          disabled={busy !== ""}
          onPress={() => void download("groups/export", "groups.csv", "export")}
        >
        ...
        <Button
          ...
          disabled={busy !== ""}
          onPress={() =>
            void download("groups/import-template", "groups-template.csv", "template")
          }
        >
        ...
        <Label muted small>
          ParentName must match an active top-level group&apos;s name; leave it
          blank for a top-level group. A parent row must appear before its
          children&apos;s rows. A row with an Id that matches an existing group
          updates it; otherwise a new group is created.
        </Label>
        <Button
